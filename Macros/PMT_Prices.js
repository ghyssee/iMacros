...
Components.utils.import("resource://gre/modules/FileUtils.jsm");
	...
	var txt = null;
	var nrOfHeaders = 1;
	logV2(INFO, "Updating configuration file ");
	var newFile = DATASOURCE_DIR + "PMT.csv";
		if (fileExists(newFile)){
			deleteFile(newFile);
		}
	do {
		var obj = getEmptyObj();
		obj.sd = getValue("SD", counter, nrOfHeaders);
		if (isNullOrBlank(obj.sd)){
			if (counter == 1){
			   break;
			}
			else {
				logV2(INFO, "NrOfHEaders = " + nrOfHeaders);
				nrOfHeaders++;
				counter=1;
				continue;
			}
			...
			if (counter == 1){
				getHeader(newFile, counter, nrOfHeaders);
				addHeader(newFile, getEmptyObj());
			}
			obj.description = getValue("desc", counter, nrOfHeaders);
			obj.ed = getValue("ED", counter, nrOfHeaders);
			obj.minRange = getValue("MinRange", counter, nrOfHeaders);
			obj.maxRange = getValue("MaxRange", counter, nrOfHeaders);
			obj.price = getValue("Price", counter, nrOfHeaders);
			obj.value = getValue("Value", counter, nrOfHeaders);
			obj.priceElementDescription = getValue("PriceElementDescription", counter, nrOfHeaders);
			obj.fixed = getValue("Fixed", counter, nrOfHeaders);
			obj.quantityType = getValue("quantityType", counter, nrOfHeaders);
			obj.valueType = getValue("valueType", counter, nrOfHeaders);
			obj.calculationType = getValue("calculationType", counter, nrOfHeaders);
			addLine(newFile, obj);
		}
		counter = counter + 1;
	}
