...
function checkOptimization(scheduledJobs){
    ...
    if ((calcExp - resources.exp) > 2000){
        ...
        var filters = [
            ...
            addFilter(JOBSELECT.SELECTTYPES.JOBTYPE, JOBSELECT.FILTER.ENERGY),
            addFilter(JOBSELECT.SELECTTYPES.CONSUMABLECOST, JOBSELECT.FILTER.NO),
            addFilter(JOBSELECT.SELECTTYPES.ENERGYRANGE, JOBSELECT.FILTER.YES, 0, resources.energyObj.left)
        ];
        var energyJobs = getJobs(jobsObj.districts, filters, !JOBSELECT_LOG, null, JOBSELECT.SORTING.EXP, JOBSELECT.SORTING.DESCENDING);
        filters = [
            addFilter(JOBSELECT.SELECTTYPES.EVENT, filterEvent()),
            addFilter(JOBSELECT.SELECTTYPES.MONEYCOST, JOBSELECT.FILTER.NO),
...
function doRobbing(){
    ...
    if (retCode == SUCCESS) {
        do {
            ...
            case JOBCONSTANTS.ROBBING.SUCCESSFUL:
                retCode = JOBCONSTANTS.ROBBING.COLLECT;
                //collectShakedown();
                break;
            case JOBCONSTANTS.ROBBING.FINISHED:
                retCode = JOBCONSTANTS.ROBBING.FINISHED;
                break;
            }
        }
        while (status != JOBCONSTANTS.ROBBING.END);
        var closed = closePopupByTextV2("Stolen Goods");
        if (!closed) {
...
function initRobbing(){
    var retCode = initAndCheckScript(JOB_FOLDER, "60_Robbing_Start", "61_Robbing_Test.iim", "?", "ROBBING", "Init Robbing");
    if (retCode != SUCCESS){
...
function chooseProperty(){
    logV2(INFO, "ROBBING", "Choose Property");
    var number = randomNumber(1, settingsObj.robbing.nrOfProperties);
    logV2(INFO, "ROBBING", "Property choosen: " + number);
    addMacroSetting("ID", number.toString());
    retCode = playMacro(JOB_FOLDER, "63_Robbing_Choose.iim", MACRO_INFO_LOGGING);
    if (retCode != SUCCESS){
